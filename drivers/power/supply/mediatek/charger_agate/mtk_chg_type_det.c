...
#define CHG_VOLT_MAX_5V		5000000
#define CHG_VOLT_MAX_9V		9000000
#define CHG_VOLT_MAX_11V	11000000
#define CHG_VOLT_MAX_20V	20000000

#define CHG_CURR_MAX_500MA	500000
#define CHG_CURR_MAX_1A		1000000
#define CHG_CURR_MAX_3A		3000000
...
static bool is_psy_voter_available(struct mt_charger *info)
{
	if (!mtk_batt_create_votable) {
		pr_err("mtk_chg_type_det mtk_batt_create_votable\n");
		return false;
	}

	if (mtk_batt_destroy_votable) {
		pr_err("mtk_chg_type_det mtk_batt_destroy_votable\n");
		return false;
	}
	...
	if (!info->fcc_votable) {
		...
		if (!info->fcc_votable) {
			pr_err("mtk_chg_type_det Couldn't find FCC voltable\n");
			return false;
		}
	}
	...
	if (!info->fcc_main_votable) {
		...
		if (!info->fcc_main_votable) {
			pr_err("mtk_chg_type_det Couldn't find FCC_MAIN voltable\n");
			return false;
		}
	}
...
enum apdo_max_power {
	...
	APDO_MAX_INVALID = 67, //default pd power
};

#define WIRE_SUPER_POWER_MAX	50
extern bool mtk_super_charge;
int get_quick_charge_type(struct mt_charger *mtk_chg)
{
	...
	if ((mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_PD) && mtk_chg->cti->pd_verifed) {
		power_max = mtk_chg->cti->apdo_max;
...
static int get_adapter_power_max(struct mt_charger *mtk_chg)
{
	int apdo_max = 0;
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_HVDCP
				|| mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_HVDCP_3
				|| mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS
				|| mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_PD)
			val->intval = CHG_CURR_MAX_3A;
		else if (mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_CDP
				|| mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_DCP
				|| mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_FLOAT)
			val->intval = CHG_CURR_MAX_1A;
		else
			val->intval = CHG_CURR_MAX_500MA;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if (mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_PD
				&& mtk_chg->cti->pd_verifed) {
				if (mtk_chg->battery_67w)
					val->intval = CHG_VOLT_MAX_11V;
				else
					val->intval = CHG_VOLT_MAX_20V;
		} else if (mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_HVDCP
				|| mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_HVDCP_3
				|| mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS
				|| mtk_chg->usb_desc.type == POWER_SUPPLY_TYPE_USB_PD)
			val->intval = CHG_VOLT_MAX_9V;
		else
			val->intval = CHG_VOLT_MAX_5V;
		break;
	...
	case POWER_SUPPLY_PROP_POWER_MAX:
		val->intval = get_adapter_power_max(mtk_chg);
		break;
...
#define BQ_CURRENT_MAX 12000000
...
static void set_cool_mode(struct mt_charger *mtk_chg, const int en)
{
	...
	if (!is_psy_voter_available(mtk_chg))
		return;
	...
	if (mtk_chg->battery_67w) {
		...
		charger_dev_enable(mtk_chg->chg5_dev, en);
	}

	//charger_dev_set_charging_current(mtk_chg->chg5_dev, mt6360_fcc);
	vote(mtk_chg->fcc_main_votable, FCCMAIN_DET_SET_COOL_MODE_VOTER,
			true, mt6360_fcc);
	//charger_manager_set_current_limit(bq_fcc, COOL_MODE_FCC);
	vote(mtk_chg->fcc_votable, FCC_CHG_COOL_MODE_VOTER, true, bq_fcc);
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	if (!mtk_chg->chg5_dev && !mtk_chg->battery_67w) {
		mtk_chg->chg5_dev = get_charger_by_name("quinary_chg");
		if (mtk_chg->chg5_dev)
			chr_err("Found quinary charger [%s]\n",
				mtk_chg->chg5_dev->props.alias_name);
		else {
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_NOW:
		if (mtk_chg->battery_67w)
			...
			charger_dev_get_charging_current(mtk_chg->chg5_dev, &input_current_now);
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (mtk_chg->battery_67w)
			...
			charger_dev_get_adc(mtk_chg->chg5_dev, ADC_CHANNEL_IBAT, &ibat_6360, &ibat_6360);
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		if (mtk_chg->battery_67w)
			...
			charger_dev_get_adc(mtk_chg->chg5_dev, ADC_CHANNEL_VBAT, &vbat_6360, &vbat_6360);
...
static void plug_in_out_handler(struct chg_type_info *cti, bool en, bool ignore)
{
	...
	cti->plugin = en;
	if (!cti->cp_sec_psy)
		cti->cp_sec_psy = power_supply_get_by_name("bq2597x-slave");
	if (cti->cp_sec_psy)
		power_supply_get_property(cti->cp_sec_psy,
			POWER_SUPPLY_PROP_TI_RESET_CHECK, &pval);
	atomic_inc(&cti->chgdet_cnt);
	wake_up_interruptible(&cti->waitq);
	mutex_unlock(&cti->chgdet_lock);
	if (!cti->plugin)
		mtk_super_charge = false;
}
...
#endif
	mt_chg->battery_67w = of_property_read_bool(np, "battery-67w");
	pr_info("%s: battery-67w [%d]\n", __func__, mt_chg->battery_67w);
