...
static int gsx_enter_gesture_mode(struct goodix_ts_device *ts_dev)
{
	...
	ts_info("enter gsx_enter_gesture_mode.\n");
	...
	struct goodix_ext_module *module)
{
	...
	core_data->start_time = ktime_get_real_ns();
	...
	ts_info("Gesture data %*ph", 5, temp_data);
...
gesture_ist_exit:
	ts_dev->hw_ops->write_trans(ts_dev, ts_dev->reg.gesture,
				&clear_reg, 1);
	core_data->end_time = ktime_get_real_ns();
	ts_info("Gesture execution time = %d ns", core_data->end_time - core_data->start_time);
	...
	struct goodix_ext_module *module)
{
	...
	ts_err("enter %s :gesture_enabled = %d\n", __func__, core_data->gesture_enabled);
