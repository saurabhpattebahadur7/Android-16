 ...
 * published by the Free Software Foundation.fts_spi4_mode_set
 *
...
int fts_chip_powercycle(struct fts_ts_info *info)
{
	...
	//disbale dvdd by switching gpio 27 pin status
...
{
	int i = 0;
	u32 const * fts_edgemode;
	const struct fts_hw_platform_data *bdata = fts_info->board;

	if (direction) {
		if (on) {
			fts_edgemode = bdata->edgezone_filter_hor;
		} else {
			fts_edgemode = bdata->normal_edgezone_filter_hor;
		}
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM / 3; i += GRIP_PARAMETER_NUM) {
			fts_set_grip_rect((int *)(fts_edgemode + i));
		}
	} else {
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM / 3; i += GRIP_PARAMETER_NUM) {
...
static void fts_corner_rejection(bool on, int direction)
{
	...
	if (direction == 1) {
		...
		if (on) {
			fts_cornermode = bdata->cornerzone_filter_hor1;
		} else {
			fts_cornermode = bdata->normal_cornerzone_filter_hor1;
		}
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM / 3; i += GRIP_PARAMETER_NUM) {
			fts_set_grip_rect((int *)(fts_cornermode + i));
		}
	}
	if (direction == 3) {
		...
		if (on) {
			fts_cornermode = bdata->cornerzone_filter_hor2;
		} else {
			fts_cornermode = bdata->normal_cornerzone_filter_hor2;
		}
		for (i = 0; i < GRIP_RECT_NUM * GRIP_PARAMETER_NUM / 3; i += GRIP_PARAMETER_NUM) {
			fts_set_grip_rect((int *)(fts_cornermode + i));
		}
...
#endif
/*
static int fts_dvdd_reg(struct fts_ts_info *info, bool enable)
{
	int retval;
	...
	if (enable) {
		if (info->ts_pinctrl && info->pinctrl_dvdd_enable) {
			...
			if (retval < 0) {
				logError(1, "%s %s: Failed to enable dvdd, error= %d\n",
					tag, __func__, retval);
				return -1;
			}
		} else {
		...
		if (info->ts_pinctrl && info->pinctrl_dvdd_disable) {
			retval = pinctrl_select_state(info->ts_pinctrl,
				info->pinctrl_dvdd_disable);
			if (retval < 0) {
				logError(1, "%s %s: Failed to disable dvdd, error= %d\n",
					tag, __func__, retval);
				return -1;
			}
		} else {
...
static bool fts_judge_hwid(void)
{
	...
	/*k2 p0, p1.1 and p1.d return true*/
	...
	/*k1/k1a return true temporary*/
	if (!strncmp(product_name, "star", 4) || !strncmp(product_name, "mars", 4))
		return 1;
	return 0;
}
#endif
...
#ifndef FTS_FOD_AREA_REPORT
	if (device_may_wakeup(dev) && (info->gesture_enabled || info->aod_status)) {
